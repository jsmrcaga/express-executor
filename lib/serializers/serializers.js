...
class BaseSerializer {
	...
	serialize_instance(instance) {
		...
		if (!available_fields || !available_fields.length) {
			return json;
		}
...
class GenericSerializer extends BaseSerializer {
	...
	get availabe_fields_list() {
		...
		const fields = this.constructor.available_fields || this.constructor.fields;
		if(!fields) {
			if(this.fields) {
	...
	get fields() {
		if(Object.keys(this.Model.VALIDATION_SCHEMA).length) {
	...
	serialize() {
		...
		if(this.many) {
			if(this.instance.some(e => !(e instanceof this.Model))) {
				throw new SerializationError(`Every object in list must be an instance of ${this.Model.name}`);
			}
		} else {
